...
async fn test_data_visibility_control() {
    ...
    // 4. Submit Block with Encrypted Data
    {
        ...
        let mut block = bc.create_block_proposal("PUBLIC_METADATA_ONLY".to_string(), "VALIDATOR".to_string())
            .expect("Failed to create block");
        
        block.encrypted_data = Some(encrypted_json.clone());
        block.hash = block.calculate_hash(); // Recalculate hash with encrypted data
        
        bc.submit_signed_block(block).expect("Failed to submit block");
    }
    ...
    let stored_encrypted_json = stored_block.encrypted_data.as_ref().expect("Should have encrypted data");
    ...
    assert!(decrypt_attempt.is_err(), "Unauthorized key should NOT decrypt data");
    ...
    assert_eq!(decrypted_data, sensitive_data, "Decrypted data must match original");
}
