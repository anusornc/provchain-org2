...
interface EnhancedTraceResponse {
  events: Array<{
    id?: string;
    timestamp?: string;
    activity?: string;
...
const ProvenanceTracker: React.FC = () => {
  ...
  const loadProvenanceData = async () => {
    ...
    try {
      ...
      const response = await productAPI.getEnhancedTrace(productId);
      const traceData: EnhancedTraceResponse = response.data;

      // Transform the trace data into provenance events
      const provenanceEvents: ProvenanceEvent[] =
        traceData.events?.map((event, index: number) => ({
          id: `event-${index}`,
      ...
      setError(
        "Failed to load provenance data: " + (responseMessage || errorMessage),
      );
      ...
      <div className="feature-header">
        <div>
          ...
          <p className="feature-description">
            Track the complete history and lineage of resources in the
            blockchain
          </p>
        </div>
        ...
        <Card title="No Data Found">
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">
              Enter a product ID above to view its provenance timeline and
              history.
            </p>
          </div>
        </Card>
