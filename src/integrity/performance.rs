...
use crate::integrity::{
    IntegrityValidationReport, IntegrityValidator, IntegrityStatus,
    BlockchainIntegrityStatus, TransactionCountIntegrityStatus, 
    SparqlIntegrityStatus, CanonicalizationIntegrityStatus
};
...
impl ValidationCache {
    ...
    pub fn insert(&mut self, key: String, result: IntegrityValidationReport) {
        ...
        self.cache.insert(key, CachedValidationResult {
            result,
    ...
    fn cleanup_expired(&mut self) {
        ...
        self.cache.retain(|_, cached| {
            now.duration_since(cached.timestamp).as_secs() < self.ttl_seconds
        });
    }
}
...
impl PerformanceMetrics {
    ...
    pub fn record_validation(&mut self, duration: Duration, used_cache: bool, used_parallel: bool, used_incremental: bool) {
...
impl OptimizedIntegrityValidator {
    ...
    pub fn new() -> Self {
        ...
        Self {
            base_validator: IntegrityValidator::with_config(false, config.max_validation_time, true),
    ...
    pub async fn validate_with_optimization(&self, blockchain: &Blockchain) -> Result<IntegrityValidationReport> {
        ...
        info!("Optimized validation completed in {:?} with level {:?}", 
    ...
    /// Standard validation for normal operations
    #[instrument(skip(self, blockchain))]
    async fn validate_standard(&self, blockchain: &Blockchain) -> Result<IntegrityValidationReport> {
        ...
        report.blockchain_integrity = self.validate_blockchain_basic(blockchain)?;
        report.transaction_count_integrity = self.base_validator.validate_transaction_counts(blockchain)?;
    ...
    /// Comprehensive validation for critical systems
    #[instrument(skip(self, blockchain))]
    async fn validate_comprehensive(&self, blockchain: &Blockchain) -> Result<IntegrityValidationReport> {
        debug!("Performing comprehensive validation");
        
        let mut report = IntegrityValidationReport::new();
        
        // All validations except full canonicalization
        report.blockchain_integrity = self.base_validator.validate_blockchain_integrity(blockchain)?;
        report.transaction_count_integrity = self.base_validator.validate_transaction_counts(blockchain)?;
        report.sparql_query_integrity = self.base_validator.validate_sparql_consistency(&blockchain.rdf_store)?;
        
        // Limited canonicalization validation for performance
        report.rdf_canonicalization_integrity = self.validate_canonicalization_sample(&blockchain.rdf_store).await?;
    ...
    fn validate_blockchain_basic(&self, blockchain: &Blockchain) -> Result<BlockchainIntegrityStatus> {
    ...
    async fn validate_canonicalization_sample(&self, rdf_store: &RDFStore) -> Result<CanonicalizationIntegrityStatus> {
        let mut status = CanonicalizationIntegrityStatus::new();
        
        // Use the canonicalization validator directly for sample validation
        let canonicalization_validator = crate::integrity::canonicalization_validator::CanonicalizationValidator::with_config(
        ...
        for graph_name in all_graphs.iter().take(sample_size) {
            match canonicalization_validator.validate_single_graph_consistency(rdf_store, graph_name) {
                Ok(result) => status.algorithm_consistency_checks.push(result),
                Err(e) => {
                    status.hash_validation_failures.push(format!(
                        "Sample canonicalization validation failed for graph {}: {}", graph_name, e
                    ));
                }
            }
        }
    ...
    pub fn update_config(&mut self, config: PerformanceConfig) {
        ...
        info!("Performance configuration updated: level={:?}, parallel={}, caching={}", 
...
impl BackgroundIntegrityService {
    ...
    #[instrument(skip(self, blockchain))]
    pub async fn start_background_monitoring(&self, blockchain: Arc<Mutex<Blockchain>>) -> Result<()> {
        if !self.enabled {
            debug!("Background monitoring is disabled");
            return Ok(());
        }

        info!("Starting background integrity monitoring with {:?} intervals", self.monitoring_interval);
        ...
        loop {
            ...
            if let Ok(blockchain_guard) = blockchain.lock() {
                match self.validator.validate_with_optimization(&*blockchain_guard).await {
                    Ok(report) => {
                        // Store last result
    ...
    pub fn set_enabled(&mut self, enabled: bool) {
        ...
        info!("Background monitoring {}", if enabled { "enabled" } else { "disabled" });
    }
...
mod tests {
    ...
    async fn test_optimized_validator_minimal() {
        ...
        assert!(report.transaction_count_integrity.counting_discrepancies.is_empty());
        assert!(report.sparql_query_integrity.query_consistency_checks.is_empty());
        assert!(report.rdf_canonicalization_integrity.algorithm_consistency_checks.is_empty());
    }
