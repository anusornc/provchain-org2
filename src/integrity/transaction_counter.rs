...
use crate::integrity::{TransactionCountIntegrityStatus, TransactionCountDetails, IntegrityRecommendation, RecommendationSeverity};
...
impl TransactionCountValidator {
    ...
    pub fn count_actual_transactions_per_block(&self, blockchain: &Blockchain) -> Result<HashMap<u64, TransactionCountDetails>> {
        let mut per_block_counts = HashMap::new();
        
        if self.verbose_logging {
            info!("Counting actual transactions for {} blocks", blockchain.chain.len());
        }
        ...
        debug!("Transaction counting completed for {} blocks", per_block_counts.len());
        Ok(per_block_counts)
    }
        ...
        if self.parse_rdf_content {
            match self.parse_rdf_content_for_transactions(&block.data) {
                Ok(count) => {
                    ...
                    if self.verbose_logging {
                        debug!("Block {}: parsed {} triples from RDF content", block.index, count);
                    }
                }
                Err(e) => {
                    details.rdf_parsing_errors.push(format!("RDF parsing failed: {}", e));
                    error!("Failed to parse RDF content for block {}: {}", block.index, e);
                }
            }
        }
    ...
    pub fn parse_rdf_content_for_transactions(&self, rdf_data: &str) -> Result<usize> {
        if rdf_data.len() > self.max_rdf_parse_size {
            return Err(crate::error::ProvChainError::Validation(
                crate::error::ValidationError::InvalidInput {
                    ...
                    reason: format!("RDF content too large: {} bytes > {} bytes limit",
                                  rdf_data.len(), self.max_rdf_parse_size)
                }
            ));
        }
        ...
        let temp_store = Store::new().map_err(|e| {
            crate::error::ProvChainError::Storage(
        ...
        match temp_store.load_from_reader(RdfFormat::Turtle, reader) {
            Ok(_) => {
                ...
                if self.verbose_logging {
                    debug!("Successfully parsed {} triples from Turtle RDF content", triple_count);
                }
            ...
            Err(turtle_error) => {
                ...
                match temp_store.load_from_reader(RdfFormat::NTriples, reader) {
                    Ok(_) => {
                        ...
                        if self.verbose_logging {
                            debug!("Successfully parsed {} triples from N-Triples RDF content", triple_count);
                        }
                    ...
                    Err(ntriples_error) => {
                        ...
                        match temp_store.load_from_reader(RdfFormat::RdfXml, reader) {
                            Ok(_) => {
                                ...
                                if self.verbose_logging {
                                    debug!("Successfully parsed {} triples from RDF/XML content", triple_count);
                                }
                            ...
                            Err(rdfxml_error) => {
                                ...
                                if self.verbose_logging {
                                    debug!("Fallback line counting: {} transaction lines", line_count);
                                }
    ...
    pub fn count_actual_rdf_triples(&self, rdf_store: &RDFStore) -> Result<usize> {
        ...
        if let oxigraph::sparql::QueryResults::Solutions(solutions) = rdf_store.query(per_block_query) {
            for solution in solutions {
                if let Ok(sol) = solution {
                    if let Some(count_term) = sol.get("count") {
                        if let oxigraph::model::Term::Literal(lit) = count_term {
    ...
    #[instrument(skip(self, blockchain))]
    pub fn validate_transaction_count_consistency(&self, blockchain: &Blockchain) -> Result<Vec<String>> {
        let mut inconsistencies = Vec::new();
        
        if self.verbose_logging {
            info!("Validating transaction count consistency for {} blocks", blockchain.chain.len());
        }
        ...
        let total_reported: usize = per_block_counts.values()
            .map(|details| details.reported_count)
            .sum();
        let total_actual: usize = per_block_counts.values()
        ...
        let blocks_with_discrepancies: Vec<_> = per_block_counts.iter()
        ...
        if blocks_with_discrepancies.len() > blockchain.chain.len() / 2 {
            inconsistencies.push(format!(
                ...
                blocks_with_discrepancies.len(), blockchain.chain.len()
            ));
        }
        
        // Check for consistent over/under counting patterns
        let over_counted: Vec<_> = per_block_counts.iter()
            .filter(|(_, details)| details.reported_count > details.actual_triple_count)
            .collect();
        let under_counted: Vec<_> = per_block_counts.iter()
            .filter(|(_, details)| details.reported_count < details.actual_triple_count)
            .collect();
        ...
        for block in &blockchain.chain {
            let block_graph_query = format!(r#"
                SELECT (COUNT(*) as ?count) WHERE {{
        ...
        if self.verbose_logging {
            if inconsistencies.is_empty() {
                info!("Transaction count consistency validation passed for {} blocks", blockchain.chain.len());
            } else {
                warn!("Found {} transaction count inconsistencies", inconsistencies.len());
            }
        }
        
        debug!("Transaction count consistency validation completed with {} inconsistencies", inconsistencies.len());
        Ok(inconsistencies)
    }

    /// Generate transaction counting recommendations
    pub fn generate_recommendations(&self, status: &TransactionCountIntegrityStatus) -> Vec<IntegrityRecommendation> {
        let mut recommendations = Vec::new();

        // Total count discrepancy recommendations
        let total_discrepancy = status.reported_total_transactions
        ...
        let blocks_with_errors: Vec<_> = status.per_block_transaction_counts.iter()
        ...
        if !blocks_with_errors.is_empty() {
            recommendations.push(IntegrityRecommendation {
                ...
                category: "Transaction Counting".to_string(),
                description: format!("Found RDF parsing errors in {} blocks", blocks_with_errors.len()),
        ...
        let blocks_with_discrepancies: Vec<_> = status.per_block_transaction_counts.iter()
            .filter(|(_, details)| details.reported_count != details.actual_triple_count)
            .collect();
        ...
        if !blocks_with_discrepancies.is_empty() {
            recommendations.push(IntegrityRecommendation {
                severity: RecommendationSeverity::Warning,
                category: "Transaction Counting".to_string(),
                description: format!("Found count discrepancies in {} blocks", blocks_with_discrepancies.len()),
                action_required: "Investigate per-block counting logic and RDF content parsing".to_string(),
                auto_fixable: true,
            });
        }
        ...
        if !status.counting_discrepancies.is_empty() {
            recommendations.push(IntegrityRecommendation {
                ...
                category: "Transaction Counting".to_string(),
                description: format!("Found {} general counting discrepancies", status.counting_discrepancies.len()),
    ...
    pub fn get_counting_statistics(&self, status: &TransactionCountIntegrityStatus) -> TransactionCountingStatistics {
        let total_blocks = status.per_block_transaction_counts.len();
        let blocks_with_errors = status.per_block_transaction_counts.iter()
            .filter(|(_, details)| !details.rdf_parsing_errors.is_empty())
            .count();
        let blocks_with_discrepancies = status.per_block_transaction_counts.iter()
            .filter(|(_, details)| details.reported_count != details.actual_triple_count)
            .count();
        
        let total_reported: usize = status.per_block_transaction_counts.values()
            .map(|details| details.reported_count)
            .sum();
        let total_actual: usize = status.per_block_transaction_counts.values()
            .map(|details| details.actual_triple_count)
            .sum();
