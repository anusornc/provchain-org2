...
use crate::ontology::error::{ValidationError, ShapeViolation, ConstraintType, ValidationResult};
...
impl Clone for ShaclValidator {
    fn clone(&self) -> Self {
        ...
        let store = Store::new().unwrap_or_else(|_| panic!("Failed to create store for ShaclValidator clone"));
...
impl ShaclValidator {
    ...
    fn load_shacl_shapes(file_path: &str) -> Result<Vec<ShaclShape>, ValidationError> {
        let content = fs::read_to_string(file_path)
            .map_err(|e| ValidationError::new(format!("Failed to read SHACL file '{}': {}", file_path, e)))?;
        ...
        store.load_from_reader(
            ...
            reader,
        ).map_err(|e| ValidationError::new(format!("Failed to parse SHACL file '{}': {}", file_path, e)))?;
        ...
        if let QueryResults::Solutions(solutions) = results {
            for solution in solutions {
                let solution = solution
                    .map_err(|e| ValidationError::new(format!("Failed to process SHACL query result: {}", e)))?;

                if let (Some(shape_term), Some(target_class_term)) = (
                    solution.get("shape"),
                    solution.get("targetClass"),
                ) {
    ...
    fn extract_shape_properties(store: &Store, shape_term: &Term) -> Result<Vec<ShaclProperty>, ValidationError> {
        ...
        let results = store.query(&query)
            .map_err(|e| ValidationError::new(format!("Failed to query shape properties: {}", e)))?;

        if let QueryResults::Solutions(solutions) = results {
            for solution in solutions {
                let solution = solution
                    .map_err(|e| ValidationError::new(format!("Failed to process property query result: {}", e)))?;
    ...
    fn extract_shape_constraints(store: &Store, shape_term: &Term) -> Result<Vec<ShaclConstraint>, ValidationError> {
        ...
        for (constraint_name, constraint_predicate) in constraint_queries {
            ...
            let results = store.query(&query)
                .map_err(|e| ValidationError::new(format!("Failed to query {} constraint: {}", constraint_name, e)))?;

            if let QueryResults::Solutions(solutions) = results {
                for solution in solutions {
                    let solution = solution
                        .map_err(|e| ValidationError::new(format!("Failed to process constraint query result: {}", e)))?;
    ...
    fn load_shapes_into_store(store: &Store, shapes: &[ShaclShape]) -> Result<(), ValidationError> {
        ...
        for shape in shapes {
            let shape_iri = NamedNode::new(&shape.id)
                .map_err(|e| ValidationError::new(format!("Invalid shape IRI '{}': {}", shape.id, e)))?;
            
            let target_class_iri = NamedNode::new(&shape.target_class)
                .map_err(|e| ValidationError::new(format!("Invalid target class IRI '{}': {}", shape.target_class, e)))?;
            ...
            )).map_err(|e| ValidationError::new(format!("Failed to insert shape triple: {}", e)))?;
            ...
            store.insert(&Quad::new(
                ...
                target_class_iri,
                oxigraph::model::GraphName::DefaultGraph,
            )).map_err(|e| ValidationError::new(format!("Failed to insert target class triple: {}", e)))?;
        }
    ...
    pub fn validate_transaction(&self, rdf_data: &str) -> Result<ValidationResult, ValidationError> {
        ...
        let data_store = Store::new()
            .map_err(|e| ValidationError::new(format!("Failed to create validation store: {}", e)))?;

        // Load the transaction RDF data
        use std::io::Cursor;
        let reader = Cursor::new(rdf_data.as_bytes());
        data_store.load_from_reader(
            oxigraph::io::RdfFormat::Turtle,
            reader,
        ...
        if violations.is_empty() {
            Ok(ValidationResult::success(constraints_checked)
                ...
                .with_metadata("core_shapes".to_string(), self.core_shapes.len().to_string())
                .with_metadata("domain_shapes".to_string(), self.domain_shapes.len().to_string()))
        } else {
    ...
    fn validate_against_shape(&self, data_store: &Store, shape: &ShaclShape) -> Result<(), Vec<ShapeViolation>> {
        let mut violations = Vec::new();
        ...
        let results = data_store.query(&query)
            .map_err(|e| vec![ShapeViolation::new(
                shape.id.clone(),
        ...
        if let Some(min_count) = property.min_count {
            if value_count < min_count {
                violations.push(ShapeViolation::new(
                    ...
                    format!("Property {} has {} values, minimum required: {}", property.path, value_count, min_count),
        ...
        if let Some(max_count) = property.max_count {
            if value_count > max_count {
                violations.push(ShapeViolation::new(
                    ...
                    format!("Property {} has {} values, maximum allowed: {}", property.path, value_count, max_count),
                ).with_property_path(property.path.clone()));
            }
        }
            ...
            for value in &values {
                ...
                if !self.validate_datatype(value, expected_datatype) {
                    violations.push(ShapeViolation::new(
                        ...
                        format!("Property {} value '{}' does not match expected datatype {}", property.path, value, expected_datatype),
                    ).with_property_path(property.path.clone()).with_value(value.clone()));
                }
            }
        }
...
mod tests {
    ...
    fn test_datatype_validation() {
        ...
        fs::write(&core_shacl_path, "@prefix sh: <http://www.w3.org/ns/shacl#> .").unwrap();
        fs::write(&domain_shacl_path, "@prefix sh: <http://www.w3.org/ns/shacl#> .").unwrap();
