                ...
                ValidationRule::BatchId => {
                    if let Some(val) = value {
                        if !BATCH_ID_REGEX.is_match(val) {
                            return Err(ValidationError::InvalidInput {
                                field: field_name.to_string(),
                                reason: "Batch ID must be 3-20 uppercase alphanumeric characters"
                                    .to_string(),
                            });
                        }
                    }
                ...
                ValidationRule::NoXss => {
                    if let Some(val) = value {
                        if val.contains('<') || val.contains('>') || val.contains("script") {
                            return Err(ValidationError::InvalidInput {
                                field: field_name.to_string(),
                                reason: "Input contains potentially dangerous HTML/script content"
                                    .to_string(),
                            });
                        }
                    }
...
impl InputValidator {
    ...
    pub fn api_validator() -> Self {
        Self::new()
            ...
            .add_field(ValidationContext::new("uri").uri().max_length(2048))
    }
}
...
mod tests {
    ...
    fn test_required_validation() {
        let validator =
            InputValidator::new().add_field(ValidationContext::new("test_field").required());

        assert!(validator.validate_field("test_field", None).is_err());
        assert!(validator.validate_field("test_field", Some("")).is_err());
        assert!(validator
            .validate_field("test_field", Some("value"))
            .is_ok());
    }

    #[test]
    fn test_length_validation() {
        let validator = InputValidator::new().add_field(
        ...
        assert!(validator
            .validate_field("test_field", Some("abcdefghijk"))
            .is_err()); // Too long
    }

    #[test]
    fn test_email_validation() {
        let validator = InputValidator::new().add_field(ValidationContext::new("email").email());

        assert!(validator.validate_field("email", Some("invalid")).is_err());
        assert!(validator
            .validate_field("email", Some("test@example.com"))
            .is_ok());
    }
    ...
    fn test_username_validation() {
        let validator =
            InputValidator::new().add_field(ValidationContext::new("username").username());

        assert!(validator.validate_field("username", Some("ab")).is_err()); // Too short
        assert!(validator
            .validate_field("username", Some("user@name"))
            .is_err()); // Invalid chars
        assert!(validator
            .validate_field("username", Some("valid_user123"))
            .is_ok());
    }
    ...
    fn test_batch_id_validation() {
        let validator =
            InputValidator::new().add_field(ValidationContext::new("batch_id").batch_id());

        assert!(validator.validate_field("batch_id", Some("ab")).is_err()); // Too short
        assert!(validator
            .validate_field("batch_id", Some("batch123"))
            .is_err()); // Lowercase
        assert!(validator
            .validate_field("batch_id", Some("BATCH123"))
            .is_ok());
    ...
    fn test_sql_injection_prevention() {
        let validator =
            InputValidator::new().add_field(ValidationContext::new("input").no_sql_injection());

        assert!(validator
            .validate_field("input", Some("SELECT * FROM users"))
            .is_err());
        assert!(validator
            .validate_field("input", Some("'; DROP TABLE users; --"))
            .is_err());
        assert!(validator
            .validate_field("input", Some("normal input"))
            .is_ok());
    }

    #[test]
    fn test_xss_prevention() {
        let validator = InputValidator::new().add_field(ValidationContext::new("input").no_xss());

        assert!(validator
            .validate_field("input", Some("<script>alert('xss')</script>"))
            .is_err());
        assert!(validator
            .validate_field("input", Some("normal input"))
            .is_ok());
    ...
    fn test_auth_validator_preset() {
        ...
        fields.insert(
            "password".to_string(),
            Some("secure_password123".to_string()),
        );
