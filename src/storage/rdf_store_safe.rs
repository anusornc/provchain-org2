...
impl crate::storage::rdf_store::RDFStore {
    ...
    pub fn add_rdf_to_graph_safe(&mut self, rdf_data: &str, graph_name: &NamedNode) -> Result<()> {
        ...
        let temp_store = Store::new()
            .map_err(|e| StorageError::ConnectionFailed(format!("Failed to create temporary store: {}", e)))?;
        ...
        match temp_store.load_from_reader(RdfFormat::Turtle, reader) {
            ...
            Err(_) => {
                ...
                let subject_uri = format!("http://provchain.org/data/{}", 
                    graph_name.as_str().replace("http://provchain.org/block/", ""));
                let subject = NamedNode::new(subject_uri)
                    .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create subject node: {}", e)))?;
                
                let predicate = NamedNode::new("http://provchain.org/hasData")
                    .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create predicate node: {}", e)))?;
                
                let object = Literal::new_simple_literal(rdf_data);
                let quad = Quad::new(subject, predicate, object, graph_name.clone());
                
                self.store.insert(&quad)
                    .map_err(|e| StorageError::QueryFailed(format!("Failed to insert fallback quad: {}", e)))?;
    ...
    pub fn load_ontology_safe(&mut self, ontology_data: &str, _graph_name: &NamedNode) -> Result<()> {
        let reader = Cursor::new(ontology_data.as_bytes());
        self.store
            .load_from_reader(RdfFormat::Turtle, reader)
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to load ontology: {}", e)))?;
    ...
    pub fn add_block_metadata_safe(&mut self, block: &Block) -> Result<()> {
        let graph_name = NamedNode::new("http://provchain.org/blockchain")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create blockchain graph name: {}", e)))?;
        
        let block_uri = NamedNode::new(format!("http://provchain.org/block/{}", block.index))
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create block URI: {}", e)))?;
        
        let data_graph_uri = NamedNode::new(format!("http://provchain.org/block/{}", block.index))
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create data graph URI: {}", e)))?;
        
        let prev_block_uri = if block.index > 0 {
            Some(NamedNode::new(format!("http://provchain.org/block/{}", block.index - 1))
                .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create previous block URI: {}", e)))?)
        } else {
            None
        };
        ...
        let block_type = if block.index == 0 {
            NamedNode::new("http://provchain.org/GenesisBlock")
                .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create genesis block type: {}", e)))?
        } else {
            NamedNode::new("http://provchain.org/Block")
                .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create block type: {}", e)))?
        };

        let type_predicate = NamedNode::new("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create type predicate: {}", e)))?;
        
        let index_predicate = NamedNode::new("http://provchain.org/hasIndex")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create index predicate: {}", e)))?;
        
        let timestamp_predicate = NamedNode::new("http://provchain.org/hasTimestamp")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create timestamp predicate: {}", e)))?;
        
        let hash_predicate = NamedNode::new("http://provchain.org/hasHash")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create hash predicate: {}", e)))?;
        
        let prev_hash_predicate = NamedNode::new("http://provchain.org/hasPreviousHash")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create previous hash predicate: {}", e)))?;
        
        let data_graph_predicate = NamedNode::new("http://provchain.org/hasDataGraphIRI")
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create data graph predicate: {}", e)))?;
        ...
        if let Some(prev) = prev_block_uri {
            let preceded_by_predicate = NamedNode::new("http://www.w3.org/ns/prov#wasPrecededBy")
                .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to create preceded by predicate: {}", e)))?;
        ...
        for quad in &quads {
            self.store.insert(quad)
                .map_err(|e| StorageError::QueryFailed(format!("Failed to insert block metadata quad: {}", e)))?;
        }
...
impl SafeRDFOperations {
    ...
    pub fn create_named_node(uri: &str) -> Result<NamedNode> {
        NamedNode::new(uri)
            .map_err(|e| StorageError::RdfParsingFailed(format!("Invalid URI '{}': {}", uri, e)).into())
    }

    /// Safely create a Store with error handling
    pub fn create_store() -> Result<Store> {
        Store::new()
            .map_err(|e| StorageError::ConnectionFailed(format!("Failed to create RDF store: {}", e)).into())
    }
    ...
    pub fn load_from_reader(store: &Store, format: RdfFormat, reader: Cursor<&[u8]>) -> Result<()> {
        store.load_from_reader(format, reader)
            .map_err(|e| StorageError::RdfParsingFailed(format!("Failed to parse RDF data: {}", e)).into())
    }
}
