...
impl PeerServer {
    ...
    async fn handle_incoming_connection(
        ...
        let _connection = PeerConnection::new_incoming(
            peer_info,
            ws_stream,
            message_handler,
        ).await?;
...
impl PeerClient {
    ...
    pub async fn connect(
        ...
        let peer_info = PeerInfo::new(
            Uuid::new_v4(), // This should be received from peer discovery
            peer_address.split(':').next().unwrap_or("unknown").to_string(),
            peer_address.split(':').nth(1).and_then(|s| s.parse().ok()).unwrap_or(0),
            "unknown".to_string(),
            false,
        );
